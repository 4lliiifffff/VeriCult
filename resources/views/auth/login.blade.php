<x-guest-layout>
    ...
    <div class="mb-12 text-center">
        ...
        <h2 class="text-4xl font-black text-[#03045E] mb-3 tracking-tight leading-tight">Selamat <span class="text-[#0077B6]">Datang</span></h2>
        <p class="text-slate-500 font-bold text-sm uppercase tracking-widest">Akses Portal VeriCult Anda</p>
    </div>
    ...
    <form method="POST" action="{{ route('login') }}" class="space-y-8">
        ...
        <div class="group">
            ...
            <div class="relative">
                ...
                <x-text-input id="email" class="block w-full pl-14 pr-5 py-4 text-sm font-bold placeholder:text-slate-300 placeholder:font-medium" type="email" name="email" :value="old('email')" required autofocus autocomplete="username" placeholder="Masukkan alamat email" />
            </div>
        ...
        <div x-data="{ show: false }" class="group">
            <div class="flex items-center justify-between mb-3 ml-1">
                <x-input-label for="password" :value="__('Kata Sandi')" class="text-[11px] font-black text-slate-400 uppercase tracking-widest transition-colors group-focus-within:text-[#0077B6]" />
                @if (Route::has('password.request'))
                    <a class="text-[10px] font-black text-[#0077B6] hover:text-[#03045E] uppercase tracking-widest transition-colors" href="{{ route('password.request') }}">
                        {{ __('Lupa?') }}
                    </a>
                @endif
            </div>
        ...
        <div class="flex items-center pt-2">
            <label for="remember_me" class="inline-flex items-center cursor-pointer group">
                <input id="remember_me" type="checkbox" class="w-5 h-5 rounded-lg border-slate-200 text-[#0077B6] shadow-sm focus:ring-[#00B4D8]/20 transition-all cursor-pointer" name="remember">
                <span class="ms-3 text-xs font-black text-slate-400 uppercase tracking-widest group-hover:text-[#0077B6] transition-colors">{{ __('Ingat Saya') }}</span>
            </label>
        </div>
        ...
        <div class="relative py-4">
            ...
            <div class="relative flex justify-center text-[10px] font-black uppercase tracking-[0.2em]">
                <span class="px-4 bg-white text-slate-300">Atau</span>
            </div>
        </div>

        <!-- Register Link -->
        <div>
            <a href="{{ route('register') }}" class="inline-flex items-center justify-center w-full border-2 border-slate-100 text-[#03045E] hover:bg-slate-50 hover:border-slate-200 font-black py-5 px-6 rounded-2xl transition-all duration-300 shadow-sm group">
                ...
                <span class="uppercase tracking-[0.2em] text-xs">{{ __('Daftar Akun Baru') }}</span>
            </a>
        </div>
    </form>
