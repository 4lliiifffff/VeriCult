<x-guest-layout>
    ...
    <div class="mb-10 text-center">
        <div class="inline-flex items-center justify-center w-16 h-16 bg-gradient-to-br from-[#0077B6] to-[#00B4D8] rounded-2xl mb-5 shadow-xl shadow-[#0077B6]/30 transition-transform duration-300 hover:scale-105">
        ...
        <h2 class="text-3xl font-bold text-[#03045E] mb-2">Selamat Datang</h2>
        <p class="text-[#64748B]">Masuk ke akun VeriCult Anda</p>
    </div>
    ...
    <form method="POST" action="{{ route('login') }}" class="space-y-6">
        ...
        <div>
            ...
            <div class="relative">
                ...
                <x-text-input id="email" class="block w-full pl-12 pr-4 py-3 text-[#03045E] bg-white" type="email" name="email" :value="old('email')" required autofocus autocomplete="username" placeholder="nama@email.com" />
            </div>
        ...
        <div class="flex items-center justify-between pt-2">
            <label for="remember_me" class="inline-flex items-center cursor-pointer group">
                <input id="remember_me" type="checkbox" class="rounded border-gray-300 text-[#0077B6] shadow-sm focus:ring-[#00B4D8] transition-colors" name="remember">
                <span class="ms-2 text-sm text-[#475569] font-medium group-hover:text-[#0077B6] transition-colors">{{ __('Ingat saya') }}</span>
            </label>

            @if (Route::has('password.request'))
                <a class="text-sm text-[#0077B6] hover:text-[#023E8A] font-semibold transition-colors duration-200" href="{{ route('password.request') }}">
                    {{ __('Lupa password?') }}
                </a>
            @endif
        </div>
        ...
        <div class="relative my-8">
            ...
            <div class="relative flex justify-center text-sm">
                <span class="px-4 bg-white text-[#64748B] font-medium">Belum punya akun?</span>
            </div>
        </div>

        <!-- Register Link -->
        <div>
            <a href="{{ route('register') }}" class="inline-flex items-center justify-center w-full border-2 border-[#0077B6] text-[#023E8A] hover:bg-[#F0F9FF] hover:border-[#0096C7] font-bold py-3.5 px-4 rounded-xl transition-all duration-300 ease-in-out shadow-sm">
                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="mt-8 pt-6 border-t border-gray-100">
        <div class="flex items-center justify-center text-xs text-gray-500">
            ...
            <span class="font-medium">Koneksi aman dengan enkripsi SSL/TLS</span>
        </div>
    </div>
</x-guest-layout>
