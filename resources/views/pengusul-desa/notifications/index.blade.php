            ...
            <div class="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6 sm:gap-8">
                <div class="flex items-center gap-4 sm:gap-8">
                    ...
                    <div class="space-y-1 sm:space-y-2">
                        <div class="flex items-center gap-2 sm:gap-3">
                            <div
                                class="px-2.5 py-1 rounded-full text-[7px] sm:text-[10px] font-black tracking-[0.2em] uppercase bg-blue-50 text-[#0077B6] border border-blue-100">
                                Pusat Notifikasi
                            </div>
                            <div class="h-3 w-[1px] bg-slate-200"></div>
                            <span
                                class="text-slate-400 text-[7px] sm:text-[10px] font-bold uppercase tracking-widest">Notifikasi</span>
                        </div>
                ...
                @if(Auth::user()->notifications->count() > 0)
                    <x-modal name="confirm-delete-all-notifications" focusable maxWidth="md">
                        <form action="{{ route('pengusul-desa.notifications.delete-all') }}" method="POST">
                            @csrf
                            @method('DELETE')

                            <div class="bg-white p-8 sm:p-12">
                                <div
                                    class="w-20 h-20 rounded-[2rem] bg-rose-50 text-rose-600 flex items-center justify-center mb-8 shadow-xl shadow-rose-900/10 border border-rose-100">
                                ...
                                <h3 class="text-3xl font-black text-[#03045E] tracking-tight leading-tight mb-4">Hapus Semua
                                    Notifikasi?</h3>
    ...
    <div x-data="{
        ...
        <div x-show="loading" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0"
            ...
            <div class="flex flex-col items-center gap-4">
                ...
                <span
                    class="text-[10px] font-black text-[#03045E] uppercase tracking-widest animate-pulse">Memperbarui...</span>
            </div>
        </div>

        <!-- Filter Bar -->
        <div
            class="max-w-4xl mx-auto mb-10 bg-white rounded-[2rem] sm:rounded-[2.5rem] p-6 sm:p-8 shadow-xl shadow-slate-200/50 border border-slate-100">
            <form x-ref="filterForm" action="{{ route('pengusul-desa.notifications.index') }}" method="GET"
                @submit.prevent="fetchNotifications()" class="auto-submit grid gap-6">
                <div class="flex flex-col">
                    <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 ml-2">Cari
                        Pesan</label>
                    <div class="relative group flex items-center">
                        ...
                        <input type="text" name="search" value="{{ request('search') }}"
                            @input.debounce.500ms="fetchNotifications()" placeholder="Ketik kata kunci..."
                ...
                <div class="flex flex-col">
                    <x-dropdown-select name="filter" id="filter" label="Status" placeholder="Pilih Status"
                        variant="light" :selected="request('filter', 'all')" :options="['all' => 'Semua', 'unread' => 'Baru', 'read' => 'Dibaca']" />
                </div>

                <div class="grid gap-4 lg:grid-cols-2">
                    <div class="flex flex-col">
                        <label
                            class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 ml-2">Dari
                            Tanggal</label>
                    ...
                    <div class="flex flex-col">
                        <label
                            class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 ml-2">Hingga</label>
    ...
    @push('scripts')
        <script>
            function markAsRead(id, button) {
                ...
                fetch(`/pengusul-desa/notifications/${id}/mark-read`, {
                    method: 'POST',
                    headers: {
