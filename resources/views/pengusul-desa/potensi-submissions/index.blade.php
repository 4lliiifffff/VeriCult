    ...
    <x-slot name="header">
            ...
            <div class="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div class="space-y-2">
                    ...
                    <h2 class="text-3xl sm:text-4xl font-black text-white tracking-tight leading-tight">
                        Potensi <span class="text-[#00B4D8]">Kebudayaan</span>
                    </h2>
                    <p class="text-blue-100/70 text-base sm:text-lg font-medium break-words">Pendataan potensi tenaga, lembaga, serta sarana dan prasarana kebudayaan di wilayah Anda.</p>
                </div>
                ...
                <div class="flex items-center gap-4 bg-white/10 backdrop-blur-xl p-3 sm:p-4 rounded-2xl border border-white/20 shadow-inner w-full md:w-auto mt-4 md:mt-0">
                    <a href="{{ route('pengusul-desa.potensi-submissions.create') }}" class="w-full md:w-auto justify-center bg-white text-[#03045E] px-4 sm:px-6 py-3 rounded-xl font-black text-[10px] sm:text-xs uppercase tracking-widest flex items-center gap-2 hover:bg-blue-50 transition-colors shadow-lg shadow-blue-900/10">
        ...
        <div class="mb-8 grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <p class="text-xs font-bold text-slate-400 uppercase tracking-wider">Total Pendataan</p>
            ...
            <div class="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-2xl font-black text-[#03045E]">{{ $submissions->whereNotIn('status', [\App\Models\CulturalSubmission::STATUS_VERIFIED, \App\Models\CulturalSubmission::STATUS_PUBLISHED, \App\Models\CulturalSubmission::STATUS_REJECTED])->count() }}</p>
                    </div>
                </div>
            </div>
            <div class="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <p class="text-xs font-bold text-slate-400 uppercase tracking-wider">Telah Terverifikasi</p>
                        <p class="text-2xl font-black text-[#03045E]">{{ $submissions->whereIn('status', [\App\Models\CulturalSubmission::STATUS_VERIFIED, \App\Models\CulturalSubmission::STATUS_PUBLISHED])->count() }}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-[2.5rem] shadow-xl shadow-slate-200/50 border border-slate-100 overflow-hidden relative group">
            <div class="p-8 border-b border-slate-50 flex items-center justify-between">
                <h3 class="font-bold text-xl text-[#03045E]">Riwayat Pendataan Potensi Kebudayaan</h3>
                    ...
                    <table class="min-w-full text-left">
                        <thead>
                            <tr class="text-[11px] font-bold text-slate-400 uppercase tracking-[0.15em]">
                                <th class="px-6 py-5">Nama Potensi</th>
                                ...
                                <td class="px-6 py-5">
                                    <div class="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <div class="text-[10px] text-slate-400 font-black uppercase tracking-widest mt-0.5">PK-{{ str_pad($submission->id, 6, '0', STR_PAD_LEFT) }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-5">
                                    @php
                                        $subCat = $submission->category_data['sub_kategori_potensi_kebudayaan'] ?? '-';
                                        $subCatLabels = [
                                            'tenaga_kebudayaan' => 'Tenaga Kebudayaan',
                                            'lembaga_kebudayaan' => 'Lembaga Kebudayaan',
                                            'sarana_prasarana' => 'Sarana & Prasarana',
                                        ];
                                        $label = $subCatLabels[$subCat] ?? $submission->category;
                                    @endphp
                                    <span class="px-3 py-1.5 rounded-xl bg-blue-50 text-[#0077B6] border-blue-100 text-[11px] font-bold tracking-wide border uppercase">
                                        {{ $label }}
                                    </span>
                                </td>
                                ...
                                <td colspan="5" class="px-6 py-32 text-center">
                                    <div class="flex flex-col items-center justify-center max-w-sm mx-auto">
                                        <div class="w-32 h-32 bg-blue-50 rounded-[2.5rem] flex items-center justify-center mb-8 relative">
                                            <svg class="w-16 h-16 text-[#0077B6]/30" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"></path></svg>
                                            <div class="absolute inset-0 border-2 border-dashed border-[#0077B6]/20 rounded-[2.5rem] animate-pulse"></div>
                                        </div>
                                        <h4 class="text-2xl font-black text-[#03045E] mb-2 tracking-tight">Belum Ada Data</h4>
                                        <p class="text-slate-400 text-sm font-medium mb-10 leading-relaxed">
                                            Anda belum mendaftarkan potensi kebudayaan di wilayah Anda. Mari mulai mendaftarkan potensi pertama!
                                        </p>
