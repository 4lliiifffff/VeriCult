<x-layouts.pengusul-desa>
    ...
    <x-slot name="header">
        <nav class="flex items-center gap-2 text-[10px] sm:text-xs font-black text-slate-400 mb-6 sm:mb-8 overflow-x-auto whitespace-nowrap pb-2 uppercase tracking-[0.2em]">
            ...
            <svg class="w-3 h-3 sm:w-3.5 sm:h-3.5 shrink-0 text-slate-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M9 5l7 7-7 7"></path></svg>
            <span class="text-[#03045E]">Ubah Potensi</span>
        </nav>
        ...
        <div class="relative bg-white rounded-[2rem] sm:rounded-[2.5rem] p-5 sm:p-10 shadow-xl shadow-slate-200/100 border border-slate-100 overflow-hidden group">
            ...
            <div class="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                <div class="flex items-center gap-4 sm:gap-8">
                    <div class="w-14 h-14 sm:w-20 sm:h-20 rounded-[1.25rem] sm:rounded-[2rem] bg-[#03045E] flex items-center justify-center text-white shadow-xl shadow-blue-900/20 font-black text-xl">
                    ...
                    <div>
                        <h2 class="text-xl sm:text-4xl font-black text-[#03045E] tracking-tight leading-tight">Ubah Potensi</h2>
                        <p class="text-slate-500 text-xs font-bold uppercase tracking-widest italic">"{{ $submission->name }}"</p>
                    </div>
                </div>
                <a href="{{ route('pengusul-desa.submissions.show', $submission) }}" class="inline-flex items-center justify-center px-8 py-4 bg-slate-50 text-slate-400 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-[#03045E] hover:text-white transition-all active:scale-95 gap-3">
            ...
            <div class="bg-white rounded-[2.5rem] sm:rounded-[3.5rem] shadow-2xl shadow-slate-200/50 border border-white overflow-hidden">
                <div class="p-8 sm:p-14">
                    <form action="{{ route('pengusul-desa.potensi-submissions.update', $submission) }}" 
                        ...
                        @include('pengusul-desa.submissions.partials.form', ['categoryFields' => $categoryFields, 'categoryName' => $submission->category, 'submission' => $submission])

                        <div class="mt-16 pt-10 border-t border-slate-50 flex flex-col sm:flex-row items-center justify-end gap-8">
                            <button type="submit" 
                                    class="w-full sm:w-auto px-12 py-5 bg-gradient-to-br from-[#03045E] to-[#0077B6] text-white font-black text-[11px] uppercase tracking-[0.2em] rounded-2xl shadow-2xl shadow-blue-900/40 hover:-translate-y-1 transition-all active:scale-95 group/submit"
                                    :disabled="loading">
                                <div class="flex items-center justify-center gap-4">
                                    <span>Simpan Perubahan</span>
                                    <svg class="w-5 h-5 group-hover/submit:translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M14 5l7 7m0 0l-7 7m7-7H3"></path></svg>
                                </div>
                            </button>
                        </div>
                    </form>
                    ...
                    <div class="relative z-10 space-y-8">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p class="text-blue-100/50 text-[10px] font-black uppercase tracking-widest">Update Potensi</p>
                            </div>
                        </div>
                        <div class="space-y-4">
    ...
    <x-modal name="confirm-submission" :show="false" focusable>
        <div class="p-10 sm:p-16 text-center">
            <div class="w-28 h-28 bg-blue-50 rounded-[2.5rem] flex items-center justify-center text-[#0077B6] mx-auto mb-10 shadow-inner group/confirm relative overflow-hidden">
                <div class="absolute inset-0 bg-[#00B4D8]/10 opacity-0 group-hover/confirm:opacity-100 transition-opacity duration-500 animate-pulse"></div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-10 border-t border-slate-50">
                <button type="button" @click="$dispatch('close')" class="px-8 py-5 rounded-2xl border-2 border-slate-100 text-slate-400 font-black text-[11px] uppercase tracking-widest hover:bg-slate-50 transition-all active:scale-95">Batal</button>
                <button type="button" @click="doSubmit()" class="px-8 py-5 rounded-2xl bg-[#03045E] text-white font-black text-[11px] uppercase tracking-widest shadow-xl shadow-blue-900/20 hover:bg-[#0077B6] transition-all active:scale-95">Ya, Simpan</button>
            </div>
        </div>
    </x-modal>
    ...
    <x-modal name="confirm-delete-file" :show="false" focusable>
        <div class="p-10 sm:p-16 text-center">
            <div class="w-28 h-28 bg-rose-50 rounded-[2.5rem] flex items-center justify-center text-rose-600 mx-auto mb-10 shadow-inner group/del">
                <svg class="w-14 h-14" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
            </div>
            <h2 class="text-4xl font-black text-rose-900 mb-4 tracking-tight leading-tight">Hapus Berkas?</h2>
            <p class="text-slate-500 max-w-xs mx-auto leading-relaxed font-bold text-sm mb-12">Berkas akan dihapus secara permanen.</p>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-10 border-t border-slate-50">
                <button type="button" @click="$dispatch('close')" class="px-8 py-5 rounded-2xl border-2 border-slate-100 text-slate-500 font-black text-[11px] uppercase tracking-widest hover:bg-slate-50 transition-all active:scale-95">Batal</button>
                <form id="deleteFileForm" method="POST">@csrf @method('DELETE') <button type="submit" class="w-full px-8 py-5 rounded-2xl bg-rose-600 text-white font-black text-[11px] uppercase tracking-widest shadow-xl shadow-rose-900/20 hover:bg-rose-700 transition-all active:scale-95">Ya, Hapus</button></form>
            </div>
        </div>
    </x-modal>
    ...
    <x-modal name="max-file-warning" :show="false" focusable>
        <div class="p-10 sm:p-16 text-center">
            ...
            <button type="button" @click="$dispatch('close')" class="w-full px-8 py-5 rounded-2xl bg-rose-600 text-white font-black text-[11px] tracking-[0.2em] uppercase shadow-[0_20px_40px_-10px_rgba(225,29,72,0.3)] hover:bg-rose-700 transition-all active:scale-[0.98]">SAYA MENGERTI</button>
        </div>
    </x-modal>
    ...
    <x-modal name="validation-warning-modal" :show="false" focusable>
        <div class="p-10 sm:p-16 text-center">
            ...
            <button type="button" @click="$dispatch('close')" class="w-full px-8 py-5 rounded-2xl bg-rose-600 text-white font-black text-[11px] tracking-[0.2em] uppercase shadow-[0_20px_40px_-10px_rgba(225,29,72,0.3)] hover:bg-rose-700 transition-all active:scale-[0.98]">KEMBALI KE FORMULIR</button>
        </div>
    </x-modal>
    ...
    <script>
        function deleteExistingFile(url) {
            ...
            if (form) { form.action = url; window.dispatchEvent(new CustomEvent('open-modal', { detail: 'confirm-delete-file' })); }
        }
            ...
            return {
                ...
                addFiles(newFiles) {
                    ...
                    if (dt.items.length + existingFilesCount > 5) {
                        ...
                        for (let i = 0; i < Math.max(dt.items.length, 5 - existingFilesCount); i++) limitedDt.items.add(dt.files[i]);
                ...
                calculateProgress() {
                    let totalQuestions = 0, filledQuestions = 0;
                    const desc = document.getElementById('description');
                    if (desc) { totalQuestions++; if (desc.value && desc.value.trim().length >= 10) filledQuestions++; }
                    const form = this.$refs.mainForm;
                    if (!form) return 0;
                    form.querySelectorAll('input[type="text"][data-category-field], textarea[data-category-field]:not(#description)').forEach(el => {
                    ...
                    return totalQuestions === 0 ? 0 : Math.min(100, Math.round((filledQuestions / totalQuestions) * 100));
                },
                ...
                openConfirm() {
                    ...
                    if (nameEl) {
                        ...
                        if (!this.submissionName || this.submissionName.trim() === '') emptyRequired.push('Identitas Umum (Nama Objek / Kebudayaan)');
                    }
                    const descEl = document.getElementById('description');
                    if (descEl) {
                        let isVisible = true, parent = descEl.parentElement;
                        while (parent && parent !== document.body) {
                            const style = window.getComputedStyle(parent);
                            if (style.display === 'none' || style.visibility === 'hidden') { isVisible = false; break; }
                            parent = parent.parentElement;
                        }
                        if (isVisible && (!descEl.value || descEl.value.trim() === '')) emptyRequired.push('Deskripsi Kebudayaan');
                    }
                ...
                doSubmit() {
                    this.loading = true;
                    this.$dispatch('close');
                    this.$nextTick(() => { (this.$refs.mainForm || this.$refs.editForm).submit(); });
                }
            }
        }
    </script>
