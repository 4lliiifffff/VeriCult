<x-layouts.super-admin>
    <x-slot name="header">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
                <h1 class="text-3xl font-black text-[#03045E] tracking-tighter mb-2">Manajemen Data Kebudayaan</h1>
                <p class="text-slate-500 font-medium">Pantau, verifikasi, dan perbarui seluruh data warisan budaya Nusantara.</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="space-y-8">
        ...
        <div class="bg-white p-6 rounded-[2rem] shadow-xl shadow-slate-200/50 border border-white">
            <form action="{{ route('super-admin.cultural-submissions.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div class="md:col-span-2">
                    <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block">Cari Nama Objek</label>
                    <div class="relative">
                        <input type="text" name="search" value="{{ request('search') }}" placeholder="Contoh: Wayang Kulit..." class="w-full pl-12 pr-4 py-3 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:border-[#0077B6] focus:ring-0 transition-all font-bold text-slate-600">
                ...
                <div>
                    ...
                    <select name="category" class="w-full px-4 py-3 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:border-[#0077B6] focus:ring-0 transition-all font-bold text-slate-600">
                ...
                <div>
                    ...
                    <select name="status" class="w-full px-4 py-3 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:border-[#0077B6] focus:ring-0 transition-all font-bold text-slate-600">
                ...
                <div class="md:col-span-4 flex justify-end">
                    <button type="submit" class="px-8 py-3 bg-[#03045E] text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-[#0077B6] transition-all shadow-lg shadow-blue-900/20">
                        Terapkan Filter
                    </button>
                    @if(request()->anyFilled(['search', 'category', 'status']))
                        <a href="{{ route('super-admin.cultural-submissions.index') }}" class="ml-4 px-8 py-3 bg-slate-100 text-slate-500 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-slate-200 transition-all">Reset</a>
                    @endif
                </div>
            </form>
        </div>
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="text-[11px] font-black text-slate-400 uppercase tracking-[0.2em] bg-slate-50/50 border-b border-slate-100">
                            <th class="px-8 py-6">Objek Kebudayaan</th>
                            <th class="px-8 py-6">Kategori</th>
                            <th class="px-8 py-6">Pengusul</th>
                    ...
                    <tbody class="divide-y divide-slate-50">
                        ...
                        <tr class="hover:bg-slate-50/50 transition-all duration-200 group">
                            <td class="px-8 py-6">
                                <div class="font-black text-[#03045E] group-hover:text-[#0077B6] transition-colors">{{ $submission->name }}</div>
