        ...
        <div class="relative bg-gradient-to-r from-[#03045E] to-[#0077B6] rounded-[2rem] p-8 overflow-hidden shadow-2xl shadow-blue-900/20">
            ...
            <div class="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div class="space-y-2">
                    ...
                    <p class="text-blue-100/70 text-lg font-medium">Lihat dan cetak hasil data kebudayaan yang sudah disetujui.</p>
                </div>
                <div class="flex items-center gap-4 bg-white/10 backdrop-blur-xl p-4 rounded-2xl border border-white/20 shadow-inner">
                    <form action="{{ route('reports.index') }}" method="GET" class="flex flex-col gap-1 items-end">
                        <label for="category" class="text-[10px] font-black text-[#00B4D8] uppercase tracking-[0.2em] pr-1">Filter Kategori</label>
                        <select name="category" id="category" onchange="this.form.submit()" class="bg-white/20 text-white border border-white/30 rounded-xl px-4 py-2 text-sm font-bold focus:ring-[#00B4D8] focus:border-[#00B4D8] transition-all outline-none cursor-pointer">
                    ...
                    <div>
                        <a href="{{ route('reports.print', ['category' => $activeCategory]) }}" target="_blank" class="bg-white text-[#03045E] px-6 py-3 rounded-xl font-black text-[10px] uppercase tracking-widest flex items-center gap-2 hover:bg-blue-50 transition-colors shadow-lg shadow-blue-900/10">
    ...
    <div class="space-y-6">
        <div class="bg-white rounded-[2.5rem] shadow-xl shadow-slate-200/50 border border-white overflow-hidden">
            <div class="p-8 border-b border-slate-50 flex items-center justify-between">
                ...
                <span class="px-4 py-1.5 bg-blue-50 text-[#0077B6] rounded-full text-xs font-bold">{{ $submissions->count() }} Data Ditemukan</span>
            </div>
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="text-[11px] font-black text-slate-400 uppercase tracking-[0.2em] bg-slate-50/50 border-b border-slate-100">
                            ...
                            <th class="px-8 py-5 text-center">Pengusul</th>
                            <th class="px-8 py-5 text-center">Tanggal Validasi</th>
                        </tr>
                    </thead>
