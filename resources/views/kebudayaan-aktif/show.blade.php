...
<body class="antialiased bg-slate-50 text-slate-900" x-data="{ 
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 -mt-20 relative z-10 pb-20">
        ...
        <div class="bg-white rounded-[1.5rem] sm:rounded-[2.5rem] shadow-2xl shadow-slate-200/80 border border-slate-100 overflow-hidden mb-8">
            ...
            <div class="p-5 sm:p-8 md:p-10 border-b border-slate-100">
                ...
                <div class="flex flex-wrap items-center gap-6 text-sm text-slate-500 font-medium">
                    <div class="flex items-center gap-2.5">
                        <div class="w-10 h-10 rounded-full bg-gradient-to-br from-[#03045E] to-[#0077B6] flex items-center justify-center text-white text-xs font-black shrink-0">
                    ...
                    @if($post->village)
                    <div class="flex items-center gap-1.5 text-[12px]">
                        <svg class="w-4 h-4 text-slate-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/></svg>
                    ...
                    <button onclick="copyLink()"
                            class="ml-auto flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-widest hover:text-[#0077B6] transition-colors group">
            ...
            @if($post->description)
            <div class="p-5 sm:p-8 md:p-10 border-b border-slate-100">
                <h2 class="text-[11px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4">Deskripsi</h2>
                <div class="prose prose-slate max-w-none text-slate-600 leading-relaxed text-[15px] font-medium">
            ...
            <div class="p-5 sm:p-8 md:p-10 border-b border-slate-100">
                <h2 class="text-[11px] font-black text-slate-400 uppercase tracking-[0.2em] mb-6">Informasi Detail</h2>
                <div class="grid sm:grid-cols-2 gap-5">
                    ...
                    @endphp
                    @if(!empty($value))
                    <div class="bg-slate-50 rounded-[1.25rem] p-4 sm:p-5 border border-slate-100">
                        <p class="text-[10px] font-black text-slate-400 uppercase tracking-[0.15em] mb-2">{{ $label }}</p>
                        <p class="text-sm font-semibold text-slate-700 leading-relaxed">{{ $value }}</p>
                    </div>
                    @endif
                    @endforeach
                </div>
            ...
            <div class="p-5 sm:p-8 md:p-10 border-b border-slate-100">
                ...
                <div class="grid grid-cols-2 {{ $images->count() > 2 ? 'sm:grid-cols-3' : '' }} gap-4">
                    @foreach($images as $img)
                    <a href="{{ $img->url }}" @click.prevent="openPreview('{{ $img->url }}', 'image', '{{ addslashes($img->original_name) }}', '{{ $img->file_size_human }}')" class="relative overflow-hidden rounded-2xl aspect-square bg-slate-100 block">
                        ...
                        <div class="absolute inset-0 bg-black/0 hover:bg-black/20 transition-colors flex items-center justify-center">
                            <svg class="w-8 h-8 text-white opacity-0 hover:opacity-100 transition-opacity" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0zM10 7v3m0 0v3m0-3h3m-3 0H7"/></svg>
                        </div>
                    </a>
                    @endforeach
                </div>
                ...
                <div class="space-y-3">
                    ...
                    <div class="flex items-center gap-4 bg-slate-50 rounded-2xl p-4 border border-slate-100">
                        <div class="w-11 h-11 bg-blue-50 rounded-xl flex items-center justify-center shrink-0">
                            <svg class="w-5 h-5 text-[#0077B6]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                        </div>
                        <div class="flex-1 min-w-0">
                            <p class="text-sm font-bold text-slate-700 truncate">{{ $doc->original_name }}</p>
                            <p class="text-[10px] text-slate-400 font-medium">{{ $doc->file_size_human }}</p>
                        </div>
                        <div class="flex items-center gap-2 shrink-0">
                            ...
                            <button type="button" @click="openPreview('{{ $doc->url }}', 'pdf', '{{ addslashes($doc->original_name) }}', '{{ $doc->file_size_human }}')"
                                    class="text-[10px] font-black text-[#0077B6] uppercase tracking-widest hover:text-[#03045E] transition-colors flex items-center gap-1.5 bg-blue-50 hover:bg-blue-100/80 px-3 py-1.5 sm:px-3.5 sm:py-2 rounded-xl border border-blue-200/50">
                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                Lihat
                            </button>
                            @endif
                            <a href="{{ $doc->url }}" target="_blank"
                               class="text-[10px] font-black text-slate-500 uppercase tracking-widest hover:text-slate-700 transition-colors flex items-center gap-1.5 bg-slate-100 hover:bg-slate-200/80 px-3 py-1.5 sm:px-3.5 sm:py-2 rounded-xl border border-slate-200/50">
                                Unduh
            ...
            {{-- Post Footer --}}
            <div class="p-5 sm:p-8 md:p-10 bg-slate-50/50 flex flex-col sm:flex-row items-center justify-between gap-4">
                <a href="{{ route('kebudayaan-aktif.index') }}"
                   class="inline-flex items-center gap-2 text-[11px] font-black text-slate-500 uppercase tracking-widest hover:text-[#03045E] transition-colors">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M15 19l-7-7 7-7"/></svg>
                    Kembali ke Feed
                </a>
                <div class="flex items-center gap-3">
                    <a href="{{ route('profil-kebudayaan.show', $post->slug) }}"
                       class="inline-flex items-center gap-2 bg-[#03045E] text-white rounded-xl px-6 py-3 text-[11px] font-black uppercase tracking-widest hover:bg-[#0077B6] transition-all active:scale-95">
        ...
        @if($related->count() > 0)
        <div>
            <h2 class="text-xl font-black text-[#03045E] tracking-tight mb-6">Kebudayaan Aktif Lainnya</h2>
