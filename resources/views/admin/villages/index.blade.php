<x-layouts.admin>
            ...
            <div class="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6 sm:gap-8">
                <div class="space-y-2 sm:space-y-3">
                    <div class="flex items-center gap-2 sm:gap-3">
                        ...
                        <span class="text-slate-400 text-[8px] sm:text-[10px] font-bold uppercase tracking-widest">Manajemen Wilayah Desa</span>
                    </div>
                    <h2 class="text-3xl sm:text-5xl font-black text-[#03045E] tracking-tight leading-tight">
                        Kelola <span class="text-[#0077B6]">Desa</span>
                    </h2>
                    <p class="text-slate-500 text-sm sm:text-lg font-medium max-w-2xl leading-relaxed">Kelola daftar desa dan pastikan pemetaan wilayah kecamatan dilakukan secara akurat.</p>
                </div>

                <div class="shrink-0 flex flex-col sm:flex-row gap-4 sm:gap-6">
                    <a href="{{ route('admin.villages.create') }}" class="group/btn px-8 py-5 bg-[#03045E] text-white rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-[#0077B6] transition-all shadow-lg shadow-blue-900/20 active:scale-95 flex items-center justify-center gap-3">
    ...
    <div x-data="{ 
        ...
        class="bg-white rounded-[2.5rem] shadow-xl shadow-slate-200/50 border border-white relative">
        <div class="p-6 sm:p-10 border-b border-slate-50 bg-white rounded-t-[2.5rem]">
            <form action="{{ route('admin.villages.index') }}" method="GET" class="flex flex-col md:flex-row gap-6 auto-submit">
                ...
                <div class="w-full md:w-80">
                    <x-dropdown-select 
                        ...
                        :options="$kecamatans->pluck('name', 'id')->toArray()" 
                    />
                </div>
                <button type="submit" class="px-8 py-4 bg-[#03045E] text-white rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-[#0077B6] transition-all shadow-lg shadow-blue-900/20 active:scale-95 md:hidden">
                    Filter
                </button>
            </form>
        </div>
                    ...
                    <tr class="hover:bg-slate-50/30 transition-colors group/u">
                        ...
                        <td class="px-6 sm:px-10 py-4 sm:py-6 text-right">
                            <div class="flex items-center justify-end gap-2">
                                <a href="{{ route('admin.villages.edit', $village) }}" class="p-2.5 rounded-xl bg-slate-50 text-slate-400 hover:bg-[#03045E] hover:text-white transition-all shadow-sm border border-slate-100">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                </a>
                                <button type="button" 
                                    @click="openDeleteModal('{{ $village->name }}', '{{ route('admin.villages.destroy', $village) }}')"
            ...
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div class="inline-block align-bottom bg-white rounded-[2.5rem] text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg w-full border border-white">
                    <div class="bg-white px-8 pt-10 pb-4 sm:p-10 sm:pb-4 text-center sm:text-left">
                        ...
                        <h3 class="text-2xl font-black text-[#03045E]" id="modal-title">Hapus Desa?</h3>
                        <p class="mt-3 text-slate-500 font-medium leading-relaxed">
                            Apakah Anda yakin ingin menghapus desa <span x-text="targetName" class="font-black text-red-600"></span>? 
                            Seluruh data terkait pada wilayah ini akan dihapus secara permanen. Tindakan ini tidak dapat dibatalkan.
                        </p>
                    </div>
