<x-layouts.admin>
        ...
        <div class="relative bg-white rounded-[2rem] sm:rounded-[2.5rem] p-6 sm:p-10 shadow-xl shadow-slate-200/100 border border-slate-100 overflow-hidden group">
            ...
            <div class="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6 sm:gap-8">
                ...
                <div class="flex items-center gap-4">
                    <a href="{{ route('admin.users.index') }}" class="px-6 py-4 bg-slate-50 text-slate-500 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-slate-200 transition-all border border-slate-100 active:scale-95">
                        Kembali
                    </a>
                </div>
            </div>
    ...
    <div x-data="{ approveModalOpen: false, rejectModalOpen: false, suspendModalOpen: false, unsuspendModalOpen: false }" class="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <p class="text-base font-black text-[#03045E]">{{ $user->profile->village->name }}</p>
                        </div>
                    </div>
                    @endif
                    ...
                    <div class="space-y-1.5 sm:col-span-2">
                        ...
                        <p class="text-base font-bold text-emerald-600">{{ \Carbon\Carbon::parse($user->profile->approved_by_admin_at)->translatedFormat('d F Y, H:i') }}</p>
                    </div>
                    @endif
                </div>
            </div>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-emerald-50 rounded-[2rem] sm:rounded-[2.5rem] border border-emerald-100 p-8 text-center">
                ...
                <p class="text-xs text-emerald-600/70 font-medium">
                    {{ $user->profile?->approved_by_admin_at ? \Carbon\Carbon::parse($user->profile->approved_by_admin_at)->translatedFormat('d F Y') : '-' }}
                </p>
            </div>
            @endif
            ...
            <div class="bg-white rounded-[2rem] sm:rounded-[2.5rem] shadow-xl shadow-slate-200/50 border border-white p-8">
                ...
                <div class="space-y-2">
                    ...
                    @if($user->is_suspended)
                        <button type="button" @click="unsuspendModalOpen = true" class="w-full flex items-center gap-3 p-4 rounded-xl bg-emerald-50 hover:bg-emerald-500 text-emerald-600 hover:text-white transition-all text-left">
                            ...
                            <span class="font-black text-xs uppercase tracking-widest">Aktifkan Akses</span>
                        </button>
                    @else
                        <button type="button" @click="suspendModalOpen = true" class="w-full flex items-center gap-3 p-4 rounded-xl bg-rose-50 hover:bg-rose-500 text-rose-600 hover:text-white transition-all text-left">
                            <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636"></path></svg>
                            <span class="font-black text-xs uppercase tracking-widest">Tangguhkan Akses</span>
                        </button>
                    @endif
                </div>
            </div>
        ...
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div class="relative z-10 inline-block align-bottom bg-white rounded-[2.5rem] text-left overflow-hidden shadow-2xl sm:my-8 sm:align-middle sm:max-w-lg w-full">
                ...
                <div class="px-8 py-8 sm:px-10 flex flex-col sm:flex-row-reverse gap-3 bg-slate-50/50">
                    <form action="{{ route('admin.user-approvals.approve', $user) }}" method="POST" class="flex-1">
                        @csrf
    ...
    <div x-show="rejectModalOpen" style="display: none;" x-cloak
        ...
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <span class="hidden sm:inline-block sm:align-middle sm:h-screen">&#8203;</span>
            <div class="relative z-10 inline-block align-bottom bg-white rounded-[2.5rem] text-left overflow-hidden shadow-2xl sm:my-8 sm:align-middle sm:max-w-lg w-full">
                <form action="{{ route('admin.user-approvals.reject', $user) }}" method="POST">
                    @csrf
                    <div class="bg-white px-8 pt-10 pb-4 sm:p-10 sm:pb-4 text-center sm:text-left">
    ...
    <div x-show="suspendModalOpen" style="display: none;" x-cloak
        ...
        class="fixed inset-0 z-[110] overflow-y-auto">
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            <div @click="suspendModalOpen = false" class="fixed inset-0 z-0 bg-slate-900/75 transition-opacity"></div>
            <span class="hidden sm:inline-block sm:align-middle sm:h-screen">&#8203;</span>
            <div class="relative z-10 inline-block align-bottom bg-white rounded-[2.5rem] text-left overflow-hidden shadow-2xl sm:my-8 sm:align-middle sm:max-w-lg w-full">
                <form action="{{ route('admin.users.suspend', $user) }}" method="POST">
                    @csrf
                    <div class="bg-white px-8 pt-10 pb-4 sm:p-10 sm:pb-4 text-center sm:text-left">
                        <div class="mx-auto sm:mx-0 flex items-center justify-center h-16 w-16 rounded-3xl bg-amber-50 text-amber-600 mb-6 italic font-black text-3xl">!</div>
                        <h3 class="text-2xl font-black text-[#03045E]">Tangguhkan User?</h3>
                        <p class="mt-3 text-slate-500 font-medium leading-relaxed">
                            Yakin ingin menangguhkan akses <span class="font-black text-amber-600">{{ $user->name }}</span>? User tidak akan bisa login.
                        </p>
                    </div>
                    <div class="px-8 py-8 sm:px-10 flex flex-col sm:flex-row-reverse gap-3 bg-slate-50/50">
                        <button type="submit" class="flex-1 inline-flex justify-center items-center px-6 py-4 bg-amber-500 hover:bg-amber-600 text-white rounded-2xl font-black text-xs uppercase tracking-[0.2em] transition-all shadow-lg shadow-amber-900/20">Ya, Tangguhkan</button>
                        <button type="button" @click="suspendModalOpen = false" class="flex-1 inline-flex justify-center items-center px-6 py-4 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-2xl font-black text-xs uppercase tracking-[0.2em] transition-all">Batal</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <div x-show="unsuspendModalOpen" style="display: none;" x-cloak
        ...
        class="fixed inset-0 z-[110] overflow-y-auto">
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            <div @click="unsuspendModalOpen = false" class="fixed inset-0 z-0 bg-slate-900/75 transition-opacity"></div>
            <span class="hidden sm:inline-block sm:align-middle sm:h-screen">&#8203;</span>
            <div class="relative z-10 inline-block align-bottom bg-white rounded-[2.5rem] text-left overflow-hidden shadow-2xl sm:my-8 sm:align-middle sm:max-w-lg w-full">
                <form action="{{ route('admin.users.unsuspend', $user) }}" method="POST">
                    @csrf
                    <div class="bg-white px-8 pt-10 pb-4 sm:p-10 sm:pb-4 text-center sm:text-left">
                        <div class="mx-auto sm:mx-0 flex items-center justify-center h-16 w-16 rounded-3xl bg-emerald-50 text-emerald-600 mb-6 font-black italic text-2xl">✓</div>
                        <h3 class="text-2xl font-black text-[#03045E]">Aktifkan User?</h3>
                        <p class="mt-3 text-slate-500 font-medium leading-relaxed">
                            Apakah Anda yakin ingin mengaktifkan kembali akses untuk <span class="font-black text-emerald-600">{{ $user->name }}</span>?
                        </p>
                    </div>
                    <div class="px-8 py-8 sm:px-10 flex flex-col sm:flex-row-reverse gap-3 bg-slate-50/50">
                        <button type="submit" class="flex-1 inline-flex justify-center items-center px-6 py-4 bg-emerald-600 hover:bg-emerald-700 text-white rounded-2xl font-black text-xs uppercase tracking-[0.2em] transition-all shadow-lg shadow-emerald-900/20">Ya, Aktifkan</button>
                        <button type="button" @click="unsuspendModalOpen = false" class="flex-1 inline-flex justify-center items-center px-6 py-4 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-2xl font-black text-xs uppercase tracking-[0.2em] transition-all">Batal</button>
                    </div>
                </form>
            </div>
        </div>
