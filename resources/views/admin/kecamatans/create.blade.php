<x-layouts.admin>
    <x-slot name="header">
        ...
        <nav class="flex items-center gap-2 text-[10px] sm:text-xs font-bold text-slate-400 mb-6 sm:mb-8 overflow-x-auto whitespace-nowrap pb-2 uppercase tracking-widest">
            ...
            <a href="{{ route('admin.kecamatans.index') }}" class="hover:text-[#0077B6] transition-colors">Data Kecamatan</a>
            <svg class="w-3 h-3 sm:w-3.5 sm:h-3.5 shrink-0 text-slate-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M9 5l7 7-7 7"></path></svg>
            <span class="text-[#03045E]">Tambah Kecamatan</span>
        </nav>
            ...
            <div class="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6 sm:gap-8">
                <div class="space-y-2 sm:space-y-3">
                    ...
                    <h2 class="text-3xl sm:text-5xl font-black text-[#03045E] tracking-tight leading-tight">
                        Tambah <span class="text-[#0077B6]">Kecamatan</span>
                    </h2>
                    <p class="text-slate-500 text-sm sm:text-lg font-medium max-w-2xl leading-relaxed">Daftarkan kecamatan baru untuk memperluas cakupan pemetaan data kebudayaan.</p>
                </div>
            </div>
        </div>
    </x-slot>
    ...
    <div class="max-w-2xl">
        <div class="bg-white rounded-[2.5rem] shadow-xl shadow-slate-200/50 border border-white p-6 sm:p-10 relative overflow-hidden group">
            ...
            <form action="{{ route('admin.kecamatans.store') }}" method="POST" class="relative z-10 space-y-8">
                @csrf

                <div class="space-y-4">
                    <label for="name" class="text-xs font-black text-slate-400 uppercase tracking-[0.2em] ml-1">Nama Kecamatan</label>
                    <input type="text" name="name" id="name" value="{{ old('name') }}" required placeholder="Masukkan nama kecamatan..."
                ...
                <div class="flex flex-col sm:flex-row gap-4 pt-4">
                    ...
                    <a href="{{ route('admin.kecamatans.index') }}" class="px-8 py-5 bg-slate-100 text-slate-500 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-slate-200 transition-all flex items-center justify-center">
                        Batal
                    </a>
                </div>
            </form>
