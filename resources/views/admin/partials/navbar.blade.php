<header class="bg-white/80 backdrop-blur-md border-b border-slate-200 sticky top-0 z-40">
    <div class="flex items-center justify-between px-4 sm:px-8 h-16">
        ...
        <div class="hidden lg:flex items-center gap-3">
            ...
            <span class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Panel Manajemen Admin</span>
        </div>
        ...
        <div class="flex items-center gap-3 sm:gap-6">
            ...
            <div class="relative" x-data="{ open: false }" @click.outside="open = false" @close.stop="open = false">
                ...
                <div x-show="open"
                    ...
                    class="fixed sm:absolute top-16 sm:top-auto inset-x-4 sm:inset-x-auto sm:right-0 z-50 mt-2 sm:mt-4 sm:w-80 rounded-2xl shadow-2xl shadow-blue-900/10 border border-slate-100 bg-white overflow-hidden"
                    style="display: none;">
                    ...
                    <div class="p-4 border-b border-slate-50 bg-slate-50/50 flex items-center justify-between">
                        ...
                        <a href="{{ route('admin.notifications.index') }}" class="text-[9px] font-black text-[#0077B6] uppercase tracking-widest hover:underline">Semua</a>
                    </div>
                    
                    <div class="max-h-96 overflow-y-auto">
                        @forelse(Auth::user()->unreadNotifications->take(5) as $notification)
                            <a href="{{ route('admin.notifications.read-and-redirect', $notification->id) }}" class="block p-4 hover:bg-blue-50 transition-colors border-b border-slate-50 last:border-0">
            ...
            <div class="relative" x-data="{ open: false }" @click.outside="open = false" @close.stop="open = false">
                <button @click="open = ! open" class="flex items-center gap-3 focus:outline-none group">
                    <div class="text-right hidden sm:block">
                        ...
                        <p class="text-[9px] font-black !text-[#00B4D8] uppercase tracking-widest leading-none">Admin Wilayah</p>
                    </div>
                ...
                <div x-show="open"
                    ...
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <a href="{{ route('logout') }}" class="flex items-center gap-3 w-full px-4 py-3 rounded-xl text-start text-xs font-black uppercase tracking-widest text-rose-400 hover:bg-rose-50 hover:text-rose-500 transition-all duration-200"
