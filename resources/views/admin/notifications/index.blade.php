<x-layouts.admin>
    <x-slot name="header">
        ...
        <div class="relative bg-white rounded-[2rem] sm:rounded-[2.5rem] p-5 sm:p-10 shadow-xl shadow-slate-200/100 border border-slate-100 overflow-hidden group">
            ...
            <div class="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6 sm:gap-8">
                ...
                <div class="flex flex-col sm:flex-row gap-3 w-full justify-end items-center">
                    @if(Auth::user()->unreadNotifications->count() > 0)
                        <form action="{{ route('admin.notifications.mark-all-read') }}" method="POST" class="w-full md:w-auto">
                            @csrf
                    ...
                    @if(Auth::user()->notifications->count() > 0)
                        <button type="button" @click="$dispatch('open-modal', 'confirm-delete-all-notifications')" class="w-full sm:w-auto sm:ml-auto px-6 py-4 bg-rose-500 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-rose-600 transition-all flex items-center justify-center gap-3 shadow-xl shadow-rose-900/20 active:scale-95">
    ...
    <div x-data="{
        ...
        <div class="max-w-4xl mx-auto mb-10 bg-white rounded-[2rem] sm:rounded-[2.5rem] p-6 sm:p-8 shadow-xl shadow-slate-200/50 border border-slate-100">
            <form x-ref="filterForm" action="{{ route('admin.notifications.index') }}" method="GET" @submit.prevent="fetchNotifications()" class="auto-submit grid gap-6">
