<x-layouts.pengusul>
                ...
                <div class="flex items-center gap-4 sm:gap-8">
                    ...
                    <div class="space-y-2 sm:space-y-3">
                        <div class="flex flex-wrap items-center gap-2 sm:gap-3">
                            <span class="px-2.5 py-1 rounded-full text-[8px] sm:text-[10px] font-black tracking-[0.15em] uppercase bg-blue-50 text-[#0077B6] border border-blue-100 shadow-sm">Mode Edit</span>
                        ...
                        <h2 class="text-xl sm:text-4xl font-black text-[#03045E] tracking-tight leading-tight break-words max-w-2xl">Ubah <span class="text-[#00B4D8]">Cagar Budaya</span></h2>
    ...
    <div x-data="submissionForm()" class="pb-20" @dragover.prevent="dragover = true" @dragleave.prevent="dragover = false" @drop.prevent="dragover = false; handleDrop($event)">
        <!-- Loading Overlay -->
            ...
            <div class="lg:col-span-8 space-y-12">
                <div class="bg-white rounded-[2.5rem] sm:rounded-[3.5rem] shadow-2xl shadow-slate-200/50 border border-white overflow-hidden group/form transition-all duration-700">
                    <div class="p-8 sm:p-14">
                        <form action="{{ route('pengusul.cagar-budaya-submissions.update', $submission) }}" method="POST" enctype="multipart/form-data" x-ref="editForm" @submit.prevent="openConfirm()">
                            @csrf
                            @method('PUT')
                            <input type="hidden" name="category" value="Cagar Budaya">
                            <input type="hidden" name="address" value="{{ $submission->address }}">
                            
                            @include('pengusul.submissions.partials.form', ['categoryFields' => $categoryFields, 'categoryName' => 'Cagar Budaya', 'submission' => $submission, 'fileDestroyRoute' => 'pengusul.cagar-budaya-submissions.files.destroy', 'hideUnesco' => true])
        ...
        <x-modal name="confirm-submission" :show="false" focusable>
            <div class="p-10 sm:p-16 text-center">
                ...
                <p class="text-slate-500 max-w-xs mx-auto font-bold text-sm leading-relaxed mb-12">Data laporan Cagar Budaya Anda akan diperbarui.</p>
        ...
        function submissionForm() {
            return {
                ...
                openConfirm() {
                    ...
                    const filesInput = document.getElementById('files');
                    if (filesInput) {
                        const hasFiles = (filesInput.files && filesInput.files.length > 0) || (this.files && this.files.length > 0) || document.querySelectorAll('[class*="group/file"]:not([x-show])').length > 0;
